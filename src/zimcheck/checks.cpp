...
std::unordered_map<MsgId, MsgInfo> msgTable = {
  ...
  { MsgId::DANGLING_LINKS,   { TestType::URL_INTERNAL, "Missing link(s) in article '{{&path}}':\n{{#links}}  - '{{&value}}' (resolves to '{{&normalized_link}}')\n{{/links}}" } },  { MsgId::EXTERNAL_LINK,    { TestType::URL_EXTERNAL, "{{&link}} is an external dependence in article {{&path}}" } },
